...
async function authorize(idOrSearchId: string) {
  ...
  let { data: search } = await supabase
    ...
    .select('id, firm_id, client_id, realtor_id, phase, kind, closing_amount')
...
const PHASE_CELEBRATIONS: Record<string, { buyer: string; seller: string }> = {
  ...
  offer_made: {
    buyer: 'Offer is in! Your agent has submitted your offer. Fingers crossed.',
    seller:
...
export async function updatePhaseAction(
  ...
  // Auto-celebrate transitions to milestone phases (kind-aware copy).
  const celebration = celebrationFor(phase, (a.search as any).kind);
