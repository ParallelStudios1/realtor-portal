-- =============================================================================
-- Realtor Portal — SETUP PART 2 (run AFTER part 1) — v3
-- Uses existing 'slug' column instead of 'subdomain'.
-- =============================================================================
...
begin
  ...
  -- generate a slug (URL-safe id) if one wasn't supplied
  v_slug := coalesce(
  ...
  -- insert the firm
  insert into public.firms (name, slug, status, created_at)
  values (trim(p_firm_name), v_slug, 'trial', now())
...
as $$
  select
    ...
    u.firm_id,
    f.name,
    f.slug,                    -- exposed as firm_subdomain to the app for naming consistency
    f.logo_url,
    f.brand_color,
    f.status,
    f.trial_ends_at,
